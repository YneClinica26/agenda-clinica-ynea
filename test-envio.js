...
function mensajeEmail(data, cita) {
  ...
  const fecha = formatoFechaES(cita.fecha);
...
@media only screen and (max-width:620px){
  ...
  .logo{width:145px!important;max-width:145px!important}
}
</style>
</head>
...
<tr><td class="detailtext" style="padding:14px 16px;font-family:Arial,Helvetica,sans-serif;color:#111827;font-size:14px;line-height:22px;">
<p style="margin:0 0 7px 0;"><strong>Fecha:</strong> ${fecha}</p>
<p style="margin:0 0 7px 0;"><strong>Hora:</strong> ${hora}</p>
<p style="margin:0 0 7px 0;"><strong>Profesional:</strong> ${profesional}</p>
<p style="margin:0;"><strong>Tipo:</strong> ${tipo}</p>
</td></tr>
</table>
</td></tr>
<tr><td class="content-pad bodytext" style="padding:0 28px 14px 28px;font-family:Arial,Helvetica,sans-serif;color:#111827;font-size:14px;line-height:21px;">
...
function normalizarTelefonoWhatsApp(telefono) {
  ...
  if (!t.startsWith("34") && t.length <= 10) t = "34" + t;
  return "whatsapp:+" + t;
}

function mensajeWhatsApp(cita, paciente) {
  return `Hola ${paciente.nombre || ""},

Le recordamos su cita en Ynea:

Fecha: ${formatoFechaES(cita.fecha)}
Hora: ${cita.hora}
Profesional: ${cita.profesional || ""}
Tipo: ${cita.tipo || ""}

Dirección:
Avenida Jacinto Benavente, 26
Valencia, 46005

Si no puede asistir, por favor comuníquenoslo respondiendo a este mensaje.

Gracias.`;
}

async function sendWhatsApp(to, mensaje) {
  if (!TWILIO_SID || !TWILIO_TOKEN || !TWILIO_WHATSAPP_FROM) {
    console.log("WhatsApp preparado pero inactivo: faltan secretos de Twilio.");
    return { skipped: true };
  }
  ...
  const body = await res.text();
  if (!res.ok) throw new Error(`Twilio error ${res.status}: ${body}`);
  return { skipped: false, body };
}

function esHoraWhatsApp48h(nowMadrid) {
  return Number(nowMadrid.hour) === 11;
}
...
async function main() {
  ...
  for (const cita of citas) {
    ...
    try {
      ...
      errores++;
      console.error(`Error enviando email cita ${cita.id}:`, err.message);
    }
  }

  const targetDate = addDaysISO(nowMadrid.date, 2);
  const citasWhatsApp = citas.filter(c => c.fecha === targetDate);

  if (!TWILIO_SID || !TWILIO_TOKEN || !TWILIO_WHATSAPP_FROM) {
    console.log("WhatsApp 48h preparado pero inactivo: faltan secretos TWILIO_SID, TWILIO_TOKEN o TWILIO_WHATSAPP_FROM.");
  } else if (!esHoraWhatsApp48h(nowMadrid)) {
    console.log(`WhatsApp 48h activo, pero no es hora de envío. Hora Madrid: ${nowMadrid.hour}:${nowMadrid.minute}`);
  } else {
    console.log(`Buscando WhatsApps 48h para citas del ${targetDate}...`);

    for (const cita of citasWhatsApp) {
      const whatsappKey = `whatsapp_48h_${cita.id}`;
      const p = pacienteDeCita(data, cita);

      if (data.avisosEnviados[whatsappKey]) {
        omitidos++;
        continue;
      }

      const to = normalizarTelefonoWhatsApp(p.telefono);
      if (!to) {
        console.log(`Sin teléfono para WhatsApp cita ${cita.id}`);
        omitidos++;
        continue;
      }

      try {
        await sendWhatsApp(to, mensajeWhatsApp(cita, p));

        data.avisosEnviados[whatsappKey] = {
          fecha: new Date().toISOString(),
          canal: "whatsapp",
        ...
        enviadosWhatsApp++;
        huboCambios = true;
        console.log(`WhatsApp enviado a ${p.telefono} para cita ${cita.id}`);
      } catch (err) {
        errores++;
        console.error(`Error enviando WhatsApp cita ${cita.id}:`, err.message);
      }
    }
  }
  ...
  console.log(`Resumen: emails=${enviadosEmail}, whatsapps=${enviadosWhatsApp}, omitidos=${omitidos}, errores=${errores}`);
}
